...
fn save_camera(camera: mat3x3f, frag_coord: vec2f, ray_origin: vec3f) -> vec4f {
    if abs(frag_coord.x - 4.5) < 0.5 { return vec4f(camera[2], -dot(camera[2], ray_origin)); }
    if abs(frag_coord.x - 3.5) < 0.5 { return vec4f(camera[1], -dot(camera[1], ray_origin)); }
    if abs(frag_coord.x - 2.5) < 0.5 { return vec4f(camera[0], -dot(camera[0], ray_origin)); }
