...
use crate::{render::Svg2d, svg::Svg};
...
pub fn apply_origin(
    ...
    for (_, svg2d_handle, origin, mut origin_state, transform, mut global_transform) in &mut query {
        let Some(svg_handle) = svg2d_handle else {
            continue;
        };
        if let Some(svg) = svgs.get(&svg_handle.0) {
